...
%
function bench = dtlz5()
M     = 2;
K     = 10;
n = M - 1 + K; 
% if n~=((M-1)+K)
%    error('The benchmark ''zdt1'' is only available for dimension %d.',(M-1)+K)
% end
% Output
bench    = struct('f'      , @(x) dtlz5Fun(x, M, K),...
                  'fDim'   , M,...
...
function F = dtlz5Front(M)
% Obtained from jMetal
F = genetic.bench.loadFrontData(['dtlz5Data' num2str(M) 'D'])';
end