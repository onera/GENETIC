...
%
if n < (K+1)
   warning('The benchmark ''wfg2'' dimension must be strictly greater or equal than %d.',K+1)
end
% Output
bench    = struct('f'      , @(x) wfg2Fun(x, M, K),...
                  'fDim'   , M,...
...
function F = wfg2Front(M)
F = genetic.bench.loadFrontData(['wfg2Data' num2str(M) 'D'])';
end
