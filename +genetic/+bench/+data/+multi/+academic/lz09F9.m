...
%
% if n ~= 30
%    warning('The standard search dimension for the benchmark ''lz09F9'' is 30.')
% end
n = 30;
% Output
bench    = struct('f'      , @(x) lz09F9Fun(x),...
                  'fDim'   , M,...
...
function y = lz09F9Fun(x)
n           = length(x);
J1          = (3:2:n)';
J2          = (2:2:n)';
y           = zeros(2,1);
y(1,1)      = x(1,1) + 2*mean((x(J1,1) - sin(6*pi*x(1,1)*ones(length(J1),1) + J1*pi/n)).^2,1);
y(2,1)      = 1 - x(1,1)^2 + 2*mean((x(J2,1) - sin(6*pi*x(1,1)*ones(length(J2),1) + J2*pi/n)).^2,1);
end

function F = lz09F9Front(M)
F = genetic.bench.loadFrontData(['lz09F9Data' num2str(M) 'D'])';
end
